...
pub contract Router {
    ...
    pub resource Mpc{
        ...
        pub fun swapIn(token:String,fromChainId:UInt64,amount:UFix64,receive: Capability<&{FungibleToken.Receiver}>){
            pre {
                Router.anyTokens.containsKey(token):
                    "Router not exists for this token"
            }
            let routerForToken=Router.anyTokens[token]!.borrow()
                ??panic("get router for capability fails")
            let receiveRef=receive.borrow()
                ??panic("get receive for capability fails")
            let swapVault<-routerForToken.mint(amount:amount)
            emit LogSwapIn(token:token,to:receive.address,amount:swapVault.balance,fromChainId:fromChainId,toChainId:Router.chainId)
